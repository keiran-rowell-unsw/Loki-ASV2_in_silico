...
for idx in {0001..3779}
do
   pdb100_bitscore=$(head -n 2  foldseek_DESULFOASV1_${idx}_pdb100.tsv | tail -1 | cut -d$'\t' -f 13)  
   swissprot_bitscore=$(head -n 2  foldseek_DESULFOASV1_${idx}_afdb-swissprot.tsv | tail -1 | cut -d$'\t' -f 13) 
   better_hit=""
   ...
   if [ $better_hit == "pdb100" ]; then
       head -n 2  foldseek_DESULFOASV1_${idx}_pdb100.tsv | tail -1 >> foldseek_DESULFOASV1_all_ORFS_top_annotated_hit.tsv
   elif [ $better_hit == "swissprot" ]; then
       head -n 2  foldseek_DESULFOASV1_${idx}_afdb-swissprot.tsv | tail -1 >> foldseek_DESULFOASV1_all_ORFS_top_annotated_hit.tsv
   elif [ $better_hit == "None" ]; then
       echo "DESULFOASV1_ORF_${idx} not PDB100 or Swiss-Prot hit" >> foldseek_DESULFOASV1_all_ORFS_top_annotated_hit.tsv
   fi       
done
...
# paste ORF_fasta_annotations.txt foldseek_DESULFOASV1_all_ORFS_top_annotated_hit.tsv > foldseek_DESULFOASV1_w_ORFS_top_annotated_hit.tsv 
#
